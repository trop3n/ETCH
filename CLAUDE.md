...
1. **Etch-stack tools** (the current focus) — a recreation/homage of the antlii.work generative toolset (artist Anatolii Babii), built on **p5.js + Tweakpane + Paper.js + opentype.js**, with a shared shell in `js/etch/`. 19 tools.
...
- **Shader** (REFRACT, BLUUR, FIELD): `startSketch` with `createCanvas(w,h,p.WEBGL)` + `setAttributes('preserveDrawingBuffer', true)` + `createShader(VERT, FRAG)` + fullscreen `rect`.
...
**Etch stack (19):** FLAKE `flake`, SPLITX `splitx`, BLUUR `bluur`, TEXTR `textr`, SAMPL `sampl`, RASTR `rastr`, RITM `ritm`, REFRACT `refract`, DITHR `dithr`, PLAIN `plain`, BIOM `biom`, DRIFT `drift`, KLON `klon`, SKAAAN `skaaan`, STIIL `stiil`, BOIDS `boids`, FIELD `field`, TERMNL `termnl`, SHAPES `shapes`.

**Legacy (11):** blob-tracker, cellular-automata, dithering, flipdigits, gradient-map, mesher, pixel-flow, pixelator, srt2video, text, video2midi.
